...
- `0.10.3`：以**修复类问题**为主，优先处理会影响数据正确性、保存、战斗面板计算的缺陷
- `0.11.0`：以**数据补全 / 规则增强 / 中等功能增强**为主
- `0.12.0`：以**较大交互增强 / 系统性功能**为主
...
- 验收标准：
  - 修改生命骰后切换角色 / 重启应用数据仍保持
  - 兼职角色的多种生命骰都可正确保存
  - 旧存档兼容逻辑不被破坏

### 2. 捡垃圾玩家包里大量垃圾武器也被计算攻击面板
- [ ] 状态：未开始
- 负责人：雪荔枝
- 目标版本：`0.10.3`
- 类型：Bug / 规则计算
- 描述：攻击面板不应把所有背包内武器都纳入可用攻击项，需明确只计算装备中、快捷栏中或符合规则条件的武器。
- 初步关联文件：
  - `src/stores/sheet/useCombatLogic.ts`
...
5. 若问题已完成，应从此处移除或标记为“已迁移到 UPDATE_LOG”
