...
> 2026-04-29 版本迭代更新：
> 当前基线已从 `0.13.11` 进入 `0.14.1`；本轮数据包系统四阶段工作栈与阶段一讨论进入 `0.14.1`。
> 2026-04-29 数据包系统工作栈：
> 数据包功能拆分为四阶段：阶段一建立数据包类型、默认数据包封装、物品库三级目录与法术库数据包来源层；阶段二实现导入/导出、启用/禁用、排序和来源筛选；阶段三实现 GM 数据包制作器；阶段四实现分级加密、密码解锁与本机解锁状态。法术库需支持按环级、学派、职业三种方式分类。
...
- `0.14.1`：记录数据包四阶段工作栈，并收口阶段一详细设计讨论。
...
- 阶段一：数据包读取与目录改造
  - 建立数据包类型系统与运行时注册表。
  - 将默认物品库封装为物品库一级菜单 `DND-5E物品仓库`。
  - 将默认法术库封装为法术库一级菜单 `DND 5E法术全书`。
  - 物品库目录升级为：数据包 / 分类 / 子分类。
  - 法术库目录升级为：数据包 / 分类方式 / 分类项；分类方式支持环级、学派、职业。
  - 先只挂载内置数据包，不做导入/导出和加密。
- 阶段二：数据包导入、启用与运行时合并
  - 支持导入/导出明文 `.dndpack.json`。
  - 支持数据包启用、禁用、删除、排序。
  - 支持物品/法术搜索结果显示数据包来源。
  - 建立 ID 命名空间与冲突检测。
- 阶段三：GM 数据包制作器
  - 支持新建/编辑数据包元信息。
  - 支持从物品库、空白模板、角色行囊快照制作数据包物品。
  - 支持从默认法术复制、空白法术模板制作数据包法术。
  - 支持导出给 PL 使用。
- 阶段四：分级加密与解锁
  - 支持数据包锁定分区。
  ...
  - 支持本机保存解锁状态，不保存明文密码。
- 待确认：
  - 阶段一是否只做内置数据包注册，不做任何文件导入。
  - 法术库三种分类方式的 UI 是切换标签，还是同时展开三个分组。
  - 默认数据包是否在内部使用同一个 pack id，但物品/法术展示两个不同菜单名。
