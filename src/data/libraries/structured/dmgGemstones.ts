...
const SOURCE = 'DMG';
const SOURCE_FILE = 'src/data/libraries/intake/dmgGemstones.ts';
const CHECKED_AT = '2026-04-27';

type ParsedGemstone = NonNullable<ItemIntakeEntry['parsed']> & {
  id: string;
  name: string;
  roll: number;
  die: string;
...
const isParsedGemstone = (entry: ItemIntakeEntry): entry is ItemIntakeEntry & { parsed: ParsedGemstone } =>
...
const gemstone = (entry: ItemIntakeEntry & { parsed: ParsedGemstone }): StructuredTreasureItem => {
  ...
  const comparedFields = [
    ...
    field('subcategory', 'gemstone', 'gemstone'),
...
export const STRUCTURED_DMG_GEMSTONES: StructuredTreasureItem[] = DMG_GEMSTONE_INTAKE
  .filter(isParsedGemstone)
  .map((entry) => gemstone(entry));

export const STRUCTURED_DMG_GEMSTONES_AUDIT_SUMMARY = {
  total: STRUCTURED_DMG_GEMSTONES.length,
  sourceMatched: STRUCTURED_DMG_GEMSTONES.filter((item) => item.audit.sourceMatched).length,
  sourceMismatched: STRUCTURED_DMG_GEMSTONES.filter((item) => !item.audit.sourceMatched).length,
  intakeTotal: DMG_GEMSTONE_INTAKE.length
};
