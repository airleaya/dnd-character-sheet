...
- **Data-pack passphrase unlock**: third-party packs can hide entries behind passphrase groups; users unlock them from Data Pack Manager or `Shift + K + L`. GM maker can edit passphrase groups and assign item/spell/trait visibility; unlock progress is persisted locally, global passphrases can make a pack fully public, exports can reset local unlock progress, and the app reports public/total information counts plus metadata warnings. This is UI visibility gating, not strong cryptographic encryption.
...
- 通过附魔制作界面编辑魔法加值、稀有度、同调、魔法视觉与魔法词条；默认魔法视觉来自预设色组序列，用户调色只覆盖当前物品实例
...
- 第三方 `.dndpack.json` 数据包保存在 Electron `userData/dnd_5e_characters/data-packs/imported/`
