...
- **Data-pack passphrase unlock**: third-party packs can hide entries behind passphrase groups; users unlock them from Data Pack Manager or `Shift + K + L`. GM maker can edit passphrase groups and assign item/spell/trait visibility; runtime unlocks can be relocked per pack or cleared for the session; the app also reports passphrase-group content counts and metadata warnings. This is UI visibility gating, not strong cryptographic encryption.
